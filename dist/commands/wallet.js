...
const sendKaspaPrompt = async (channel, userId) => {
    ...
    try {
        ...
        const confirmEmbed = new EmbedBuilder()
            ...
            .addFields({ name: 'Amount', value: `${amount} KAS` }, { name: 'Recipient Address', value: address });
...
const checkBalance = async (channel, userId) => {
    ...
    try {
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields({ name: 'Kaspa Balance', value: `${kaspaBalance} KAS` }, { name: 'KRC20 Balances', value: krc20Balances.length > 0 ? krc20Balances.join('\n') : 'No KRC20 tokens found or balance retrieval restricted' })
            .setFooter({ text: `Network: ${userSession.network}` });
